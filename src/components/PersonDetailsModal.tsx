...
interface PersonDetailsModalProps {
  ...
  onClose: () => void;
  person: Person | null;
  allPeople: Person[]; // Needed to look up Padrinho name
}

// Reusable date formatter (or import from a shared utils file)
const formatDate = (dateStr: string | undefined, format: 'year' | 'month' | 'day'): string => {
    if (!dateStr || typeof dateStr !== 'string') return 'N/A';
    try {
        const date = new Date(dateStr + 'T00:00:00'); // Use date part only
...
const PersonDetailsModal: React.FC<PersonDetailsModalProps> = ({ isOpen, onClose, person, allPeople }) => {
  ...
  const getPadrinhoName = (padrinhoId: string | undefined): string => {
    if (!padrinhoId) return 'N/A';
    const padrinho = allPeople.find(p => p.id === padrinhoId);
    return padrinho?.name || `Unknown (ID: ${padrinhoId})`;
  };

  const displayName = person.nickname ? `${person.name} "${person.nickname}"` : person.name;
  const padrinhoName = getPadrinhoName(person.padrinhoId);
  const otherInstrumentsText = person.otherInstruments?.join(', ') || 'None';

  return (
    <div style={styles.overlay} onClick={onClose}> {/* Close on overlay click */}
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}> {/* Prevent closing when clicking modal content */}
        <button onClick={onClose} style={styles.closeButton}>×</button>
        {/* Modal Content */}
        <div style={styles.modalContent}>
            <img
                src={person.imageUrl || placeholderImageUrl} // Use placeholder if URL is missing
                alt={displayName}
                style={styles.image}
                onError={(e) => { // Fallback if URL fails
                    const t=e.target as HTMLImageElement;
                    if(t.src!==placeholderImageUrl) t.src=placeholderImageUrl;
                 }}
            />
           ...
           <div style={styles.detailsGrid}>
                ...
                {person.padrinhoId && <><span style={styles.label}>Padrinho/Madrinha:</span> <span>{padrinhoName}</span></>}
           </div>
           {person.notes && (
